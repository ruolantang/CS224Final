...
Shader "waterColor/waterColorBanana"
{
	Properties
	{
		_MainTex ("Main TextureBanana", 2D) = "white" {}
		_Color ("Color Tint", Color) = (1, 1, 1, 1)
		_size ("blurSize", int) = 10
		_sigma("sigma", float) = 2.0
		_bluramount("blurAmount", float) = 0.001
		_PaintTex ("Paint TextureBanana", 2D) = "white" {}
		_PaperTex("Paaper TextureBanana", 2D) = "white" {}
	...
	SubShader
	{
            ...
            half4 frag(v2fA i) : SV_Target
            {
				...
				for (int itx = -size; itx < size + 1; itx++) {
					...
					//for (int ity = -0; ity < 0 + 1; ity++) {
						...
						bgcolor += pdf1*pdf2 * tex2Dproj(_ColorTextureBanana, UNITY_PROJ_COORD(float4(i.grabPos.x + itx*bluramount, i.grabPos.y + ity*bluramount, i.grabPos.z, i.grabPos.w)));
					}

				}
				//fixed4 noise = tex2D(_PaintTex, i.uv);
               // bgcolor = tex2Dproj(_ColorTextureBanana, i.uv);
               return bgcolor;
		...
		Pass
		{
			...
			half4 frag(v2fA i) : SV_Target
			{
				half4 bgcolor = tex2Dproj(_ColorTextureBanana, i.grabPos);
		...
		Pass
		{
			...
			sampler2D _BackgroundTextureBanana;
			sampler2D _ColorTextureBanana;
			sampler2D _BlurTextureBanana;
			sampler2D _PaperTextureBanana;
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				fixed4 bg = tex2Dproj(_BackgroundTextureBanana, i.grabPos);
				fixed4 color = tex2Dproj(_ColorTextureBanana, i.grabPos);
				fixed4 blur = tex2Dproj(_BlurTextureBanana, i.grabPos);
				fixed4 paper = tex2Dproj(_PaperTextureBanana, i.grabPos);
