...
Shader "waterColor/waterColorTable"
{
	Properties
	{
		_MainTex ("Main TextureTable", 2D) = "white" {}
		_Color ("Color Tint", Color) = (1, 1, 1, 1)
		_size ("blurSize", int) = 10
		_sigma("sigma", float) = 2.0
		_bluramount("blurAmount", float) = 0.001
		_PaintTex ("Paint TextureTable", 2D) = "white" {}
		_PaperTex("Paaper TextureTable", 2D) = "white" {}
	...
	SubShader
	{
            ...
            half4 frag(v2fA i) : SV_Target
            {
				...
				for (int itx = -size; itx < size + 1; itx++) {
					...
					//for (int ity = -0; ity < 0 + 1; ity++) {
						...
						bgcolor += pdf1*pdf2 * tex2Dproj(_ColorTextureTable, UNITY_PROJ_COORD(float4(i.grabPos.x + itx*bluramount, i.grabPos.y + ity*bluramount, i.grabPos.z, i.grabPos.w)));
					}

				}
				//fixed4 noise = tex2D(_PaintTex, i.uv);
               // bgcolor = tex2Dproj(_ColorTextureTable, i.uv);
               return bgcolor;
		...
		Pass
		{
			...
			half4 frag(v2fA i) : SV_Target
			{
				half4 bgcolor = tex2Dproj(_ColorTextureTable, i.grabPos);
		...
		Pass
		{
			...
			sampler2D _BackgroundTextureTable;
			sampler2D _ColorTextureTable;
			sampler2D _BlurTextureTable;
			sampler2D _PaperTextureTable;
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				fixed4 bg = tex2Dproj(_BackgroundTextureTable, i.grabPos);
				fixed4 color = tex2Dproj(_ColorTextureTable, i.grabPos);
				fixed4 blur = tex2Dproj(_BlurTextureTable, i.grabPos);
				fixed4 paper = tex2Dproj(_PaperTextureTable, i.grabPos);
