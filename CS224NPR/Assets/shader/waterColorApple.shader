...
Shader "waterColor/waterColorApple"
{
	Properties
	{
		_MainTex ("Main TextureApple", 2D) = "white" {}
		_Color ("Color Tint", Color) = (1, 1, 1, 1)
		_size ("blurSize", int) = 10
		_sigma("sigma", float) = 2.0
		_bluramount("blurAmount", float) = 0.001
		_PaintTex ("Paint TextureApple", 2D) = "white" {}
		_PaperTex("Paaper TextureApple", 2D) = "white" {}
	...
	SubShader
	{
            ...
            half4 frag(v2fA i) : SV_Target
            {
				...
				for (int itx = -size; itx < size + 1; itx++) {
					...
					//for (int ity = -0; ity < 0 + 1; ity++) {
						...
						bgcolor += pdf1*pdf2 * tex2Dproj(_ColorTextureApple, UNITY_PROJ_COORD(float4(i.grabPos.x + itx*bluramount, i.grabPos.y + ity*bluramount, i.grabPos.z, i.grabPos.w)));
					}

				}
				//fixed4 noise = tex2D(_PaintTex, i.uv);
               // bgcolor = tex2Dproj(_ColorTextureApple, i.uv);
               return bgcolor;
		...
		Pass
		{
			...
			half4 frag(v2fA i) : SV_Target
			{
				half4 bgcolor = tex2Dproj(_ColorTextureApple, i.grabPos);
		...
		Pass
		{
			...
			sampler2D _BackgroundTextureApple;
			sampler2D _ColorTextureApple;
			sampler2D _BlurTextureApple;
			sampler2D _PaperTextureApple;
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				fixed4 bg = tex2Dproj(_BackgroundTextureApple, i.grabPos);
				fixed4 color = tex2Dproj(_ColorTextureApple, i.grabPos);
				fixed4 blur = tex2Dproj(_BlurTextureApple, i.grabPos);
				fixed4 paper = tex2Dproj(_PaperTextureApple, i.grabPos);
